...
image = imread('D:\Programing\PhaseRetrieval_Programs\phasepack-matlab-master\data\Emacs_512.png'); % Load the image from the 'data' folder.
image = double(rgb2gray(image)); % convert image to grayscale
num_fourier_masks = 3;           % Select the number of Fourier masks
...
opts.tol = 1e-4;              % The tolerance - make this smaller for more accurate solutions, or larger for faster runtimes
...
function measurements = A(pixels)
    ...
    % back to a 2D image.
    %size(pixels)
    im = reshape(pixels,[numrows,numcols]); %[numrows,numcols]
    ...
    for m = 1:num_fourier_masks
        ...
        measurements(m,:,:) = fft2(im.*this_mask); %オーバーサンプリングしてないですね？
    end
